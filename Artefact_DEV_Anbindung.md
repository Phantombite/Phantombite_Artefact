...
## Konkretes Beispiel — Phantombite Artefact ist angebunden

Artefact ist der erste Mod der vollständig an Core angebunden wurde. Hier ist die genaue Verbindung:

### Dateien im Artefact-Mod

```
Phantombite_Artefact/
└── Data/Scripts/PhantombiteArtefact/
    ├── Core/
    │   ├── IModule.cs              (identisch wie Core-Version)
    │   ├── ModuleManager.cs        (identisch wie Core-Version)
    │   └── Session.cs              (lädt nur Artefact_Command)
    └── Modules/
        ├── Artefact_Command.cs     (Registrierung + Command-Empfang)
        └── Artefact_Controller.cs  (MyGameLogicComponent am Block)
```

### Artefact_Command.cs — Verbindung zum Core

Artefact hört auf Kanal **1995001** und registriert diese Commands beim Core:
...
Alle Commands sind `adminOnly=1` → Artefact ist ein reiner Admin-Mod, für normale Spieler unsichtbar.

### Artefact_Controller.cs — Der Block-Script

`Artefact_Controller` ist ein `MyGameLogicComponent` der auf dem SpaceArtefact-Block läuft. Er ist **nicht** direkt mit dem Core verbunden — er bekommt Commands über `Artefact_Command` weitergeleitet:

```
Core sendet: "CMD|reset|1|STEAM:76561198xxxxxxx"
       ↓
Artefact_Command.OnCommandReceived() parst die Nachricht
       ↓
Artefact_Command.ExecuteOnBlocks() sucht alle SpaceArtefact-Blöcke
       ↓
ArtefactController.ExecuteCommand("reset", targetId=1, hasId=true, allTarget=false)
       ↓
Block mit ID=1 führt Reset aus
       ↓
Artefact_Command sendet CMDRESULT zurück:
"CMDRESULT|artefact|reset|1|76561198xxxxxxx|ok|Artefact ID 1: reset ausgeführt"
       ↓
Core zeigt HUD: "[PB] Artefact ID 1: reset ausgeführt" (grün)
```

### ID-System im Artefact

Jeder SpaceArtefact-Block hat eine Custom Data mit einer ID:
...
`Artefact_Command.ExecuteOnBlocks()` iteriert alle Blöcke und filtert:
- `!pbc artefact reset` → nur ID=0
- `!pbc artefact 1 reset` → nur ID=1
- `!pbc artefact all reset` → alle IDs

### Logging im Artefact

Artefact_Command hat eine eigene Log-API die intern den Log-Level prüft:

```csharp
_logger?.Debug("Artefact_Controller", "SPIELER-TRIGGER — Distanz: 42.1m");
```

Das `_logger` ist die Artefact_Command Instanz. Sie prüft `_logLevel` und sendet nur wenn nötig:

```
"LOG|Phantombite_Artefact|DEBUG|Artefact_Controller|SPIELER-TRIGGER — Distanz: 42.1m"
```

Core empfängt auf 1995999, prüft nochmal ob Debug-Level für Phantombite_Artefact aktiv ist und schreibt in den Phantombite-Log.

### Alien-Nachrichten bleiben im Controller

Die Alien-Nachrichten (`ShowNotification` mit den Alien-Texten) bleiben im `Artefact_Controller` — das ist Gameplay-Feedback, kein Command-Feedback. Nur die Command-Bestätigungen (`[Artefakt 0] Aktiviert` etc.) wurden entfernt weil Core das übernimmt.

### Warum kein ShowNotification für Commands im Controller?

Früher hatte der Controller eigene `ShowNotification` Aufrufe nach jedem Command. Das Problem: Core weiß dann nicht ob der Command erfolgreich war. Mit dem CMDRESULT-System gibt es eine saubere Trennung:

- **Controller** → führt aus, kein Feedback
- **Artefact_Command** → meldet Ergebnis an Core
- **Core** → zeigt HUD für den richtigen Spieler

So funktioniert das Feedback auch korrekt wenn mehrere Spieler gleichzeitig Commands eingeben.